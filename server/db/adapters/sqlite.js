...
export class SqliteAdapter {
    ...
    async findUserById(id) {
        const row = this.db.prepare("SELECT id, email, name, role, avatar, created_at FROM users WHERE id = ?").get(id);
        if (!row)
            return null;
        return row;
    }
    ...
    async findSessionByToken(token) {
        const row = this.db.prepare("SELECT * FROM sessions WHERE token = ? AND expires_at > datetime('now')").get(token);
        if (!row)
            return null;
        return row;
    }
    ...
    async listAgents(userId) {
        const rows = this.db.prepare("SELECT * FROM agents WHERE user_id = ? ORDER BY created_at ASC").all(userId);
    ...
    async listAuditLogs(params) {
        ...
        const total = this.db.prepare(`SELECT COUNT(*) as count FROM audit_logs ${where}`).get(...values).count;
        const rows = this.db.prepare(`SELECT * FROM audit_logs ${where} ORDER BY created_at DESC LIMIT ? OFFSET ?`).all(...values, limit, offset);
    ...
    async listApprovals(params) {
        ...
        const total = this.db.prepare(`SELECT COUNT(*) as count FROM approvals ${where}`).get(...values).count;
        const rows = this.db.prepare(`SELECT * FROM approvals ${where} ORDER BY created_at DESC LIMIT ? OFFSET ?`).all(...values, limit, offset);
    ...
    async getStats(userId) {
        const totalAgents = this.db.prepare("SELECT COUNT(*) as count FROM agents WHERE user_id = ?").get(userId).count;
        const activeAgents = this.db.prepare("SELECT COUNT(*) as count FROM agents WHERE user_id = ? AND status = 'Online'").get(userId).count;
        const totalLogs = this.db.prepare("SELECT COUNT(*) as count FROM audit_logs WHERE user_id = ?").get(userId).count;
        const successLogs = this.db.prepare("SELECT COUNT(*) as count FROM audit_logs WHERE user_id = ? AND status = 'success'").get(userId).count;
        const pendingApprovals = this.db.prepare("SELECT COUNT(*) as count FROM approvals WHERE user_id = ? AND status = 'pending'").get(userId).count;
        return {
